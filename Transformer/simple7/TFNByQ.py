...
# 创建模型
model = TransformerModel(input_dim=7, hidden_dim=64, output_dim=7, nhead=1)
...
# 保存模型
torch.save(model.state_dict(), "transformer_model_byQ.pth")
...
# 加载模型（用于后续使用）
model_loaded = TransformerModel(input_dim=7, hidden_dim=64, output_dim=7, nhead=1)
model_loaded.load_state_dict(torch.load("transformer_model_byQ.pth"))
model_loaded.eval()
