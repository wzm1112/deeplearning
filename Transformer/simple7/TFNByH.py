...
# 加载数据并处理
file_path = '../../datasets/es[7]-vs[6]-random-train.csv'
data = pd.read_csv(file_path)

# 特征选择与标准化
selected_features = ["h'1", "h'2", "h'3", "h'4", "h'5", "h'6", "h'7"]
target_columns = ["ei"]
...
# 创建模型
model = TransformerModel(input_dim=7, hidden_dim=32, output_dim=7, nhead=1)
...
# 保存模型
torch.save(model.state_dict(), "transformer_model_byH.pth")
...
# 加载模型（用于后续使用）
model_loaded = TransformerModel(input_dim=7, hidden_dim=32, output_dim=7, nhead=1)
model_loaded.load_state_dict(torch.load("transformer_model_byH.pth"))
model_loaded.eval()
